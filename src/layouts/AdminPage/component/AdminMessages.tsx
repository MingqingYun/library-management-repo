...
export const AdminMessages = () => {
    ...
    useEffect(() => {
        const fetchUserMessages = async () => {
            if (authState && authState.isAuthenticated) {
                const url = `${BASEURL.url}/messages/search/findByClosed/?closed=false&page=${currentPage - 1}&size=${messagesPerPage}`;
    ...
    async function submitResponseToQuestion(id:number, response:string) {
        const url=`${BASEURL.url}/messages/secure/admin/message`;
            ...
            {messages.length > 0 ? 
                <>
                    ...
                    {messages.map(message => (
                       <AdminMessage message={message} submitResponseToQuestions={submitResponseToQuestion} />
                    ))}
                </>
                :
