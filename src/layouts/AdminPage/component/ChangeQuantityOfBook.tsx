...
export const ChangeQuantityOfBook: React.FC<{ book: BookModel, deleteBook:any}> = (props, key) => {
    ...
    async function increaseQuantity() {
        ...
        if (authState && authState.isAuthenticated) {
            const url: string = `${BASEURL.url}/admin/secure/increase/book/quantity?BookId=${props.book?.id}`;
    ...
    async function decreaseQuantity() {
        ...
        if (authState && authState.isAuthenticated) {
            const url: string = `${BASEURL.url}/admin/secure/decrease/book/quantity?BookId=${props.book?.id}`;
            const requestOptions = {
                method: 'PUT',
                headers: {
                    Authorization: `Bearer ${authState.accessToken?.accessToken}`,
    ...
    async function deleteBook() {
        ...
        if (authState && authState.isAuthenticated) {
            const url: string = `${BASEURL.url}/admin/secure/delete/book?BookId=${props.book?.id}`;
            const requestOptions = {
                method: 'DELETE',
                headers: {
                    Authorization: `Bearer ${authState.accessToken?.accessToken}`,
